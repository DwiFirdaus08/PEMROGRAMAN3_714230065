...
func TestGetMahasiswaByNPM(t *testing.T) {
	...
	if err != nil {
		t.Errorf("Terjadi kesalahan saat mengambil Mahasiswa: %v", err)
	} else if mhs.NPM != npm {
		t.Errorf("Diharapkan NPM %v, didapatkan %v", npm, mhs.NPM)
	} else {
		fmt.Printf("Data mahasiswa yang diambil: %+v", mhs)
	}
}
