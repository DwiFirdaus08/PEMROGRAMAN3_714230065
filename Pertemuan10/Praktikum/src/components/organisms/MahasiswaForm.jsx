...
export function MahasiswaForm({
  ...
  const [formData, setFormData] = useState({
    ...
    mataKuliah:
      initialValues.mata_kuliah
        ?.map((mk) => `${mk.nama}|${mk.kode}|${mk.nilai}`)
        .join(", ") || "",
  });
  ...
  const validate = () => {
    ...
    if (!formData.alamat.kelurahan)
      newErrors["alamat.kelurahan"] = "Kelurahan wajib diisi";
  ...
  const handleSubmit = async (e) => {
    ...
    <div style="text-align: left;">
        ...
        <b>Alamat:</b> ${formatted.alamat.jalan}, Kel. ${
      formatted.alamat.kelurahan
    }, Kota ${formatted.alamat.kota}<br/>
        <b>Minat:</b> ${formatted.minat.join(", ")}<br/>
        <b>Mata Kuliah:</b> <ul>${formatted.mata_kuliah
          .map((mk) => `<li>${mk.nama} (${mk.kode}) - ${mk.nilai}</li>`)
          .join("")}</ul>
    </div>
`;
  ...
  return (
    ...
    >
      ...
      <TextFieldAtom
        ...
        onChange={handleChange}
        error={!!errors["alamat.jalan"]}
        helperText={errors["alamat.jalan"] || " "}
      />
      ...
      <TextFieldAtom
        ...
        onChange={handleChange}
        error={!!errors.minat}
        helperText={errors.minat || " "}
      />
      <TextFieldAtom
        label="Mata Kuliah (format: NAMA | KODE | NILAI)"
        name="mataKuliah"
        value={formData.mataKuliah}
        onChange={handleChange}
        error={!!errors.mataKuliah}
        helperText={errors.mataKuliah || " "}
      />
